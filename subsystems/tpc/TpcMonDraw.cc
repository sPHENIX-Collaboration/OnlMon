  ...
  for (int i = 0; i < 24; i++)
  {
    ...
    for (int j = 2; j > -1; j--)
    {
      if (tpcmon_RAWADC1D[i][j])
      {
        if (j == 2)
        {
          tpcmon_RAWADC1D[i][j]->DrawCopy("HIST");
        }
        else
        {
          tpcmon_RAWADC1D[i][j]->DrawCopy("HISTsame");
  ...
  for (int i = 0; i < 24; i++)
  {
    ...
    for (int j = 2; j > -1; j--)
    {
      if (tpcmon_MAXADC1D[i][j])
      {
        if (j == 2)
        {
          tpcmon_MAXADC1D[i][j]->DrawCopy("HIST");
        }
        else
        {
          tpcmon_MAXADC1D[i][j]->DrawCopy("HISTsame");
        }  // assumes that R3 will always exist and is max
      }
    }
    gPad->Update();
    gPad->SetLogy(kTRUE);
  ...
  for (int i = 0; i < 24; i++)
  {
    ...
    for (int j = 2; j > -1; j--)
    {
      if (tpcmon_PEDESTSUBADC1D[i][j])
      {
        if (j == 2)
        {
          tpcmon_PEDESTSUBADC1D[i][j]->DrawCopy("HIST");
        }
        else
        {
          tpcmon_PEDESTSUBADC1D[i][j]->DrawCopy("HISTsame");
        }  // assumes that R3 will always exist and is most entries
      }
    }
    gPad->Update();
  ...
  for (int i = 0; i < 24; i++)
  {
    ...
    for (int j = 2; j > -1; j--)
    {
      if (tpcmon_DriftWindow[i][j])
      {
        for (int k = 1; k < tpcmon_DriftWindow[i][j]->GetNbinsX(); k++)  // old laser peak 416, new laser peak 403, new laser peak w/ 57.14 ns clock 368
        {
          ...
          if ((tpcmon_DriftWindow[i][j]->GetBinContent(k) > no_laser_max && tpcmon_DriftWindow[i][j]->GetBinContent(k) > 0) && (no_laser_window == 1))
          {
            no_laser_max = tpcmon_DriftWindow[i][j]->GetBinContent(k);
          }
          if (tpcmon_DriftWindow[i][j]->GetBinContent(k) > max && tpcmon_DriftWindow[i][j]->GetBinContent(k) > 0)
          {
            max = tpcmon_DriftWindow[i][j]->GetBinContent(k);
          }
          if (tpcmon_DriftWindow[i][j]->GetBinContent(k) < min && tpcmon_DriftWindow[i][j]->GetBinContent(k) > 0)
          {
            min = tpcmon_DriftWindow[i][j]->GetBinContent(k);
          }
        }
      }
    }
    ...
    for (int l = 2; l > -1; l--)
    {
      if (tpcmon_DriftWindow[i][l])
      {
        ...
        if (l == 2)
        {
          tpcmon_DriftWindow[i][l]->GetYaxis()->SetRangeUser(0.9 * min, 1.1 * max);
          tpcmon_DriftWindow[i][l]->DrawCopy("HIST");
        }  // used to be
        else
        {
          tpcmon_DriftWindow[i][l]->GetYaxis()->SetRangeUser(0.9 * min, 1.1 * max);
          tpcmon_DriftWindow[i][l]->DrawCopy("HISTsame");
        }  // assumes that R3 will always exist and is most entries
      }
    }
...
int TpcMonDraw::DrawTPCNStreaksvsEventNo(const std::string & /* what */)
{
  ...
  for (int i = 0; i < 24; i++)
  {
    if (tpcmoneventsebdc[i])
    {
      if (tpcmoneventsebdc[i]->GetBinContent(i + 1) > event_max)
      {
        event_max = tpcmoneventsebdc[i]->GetBinContent(i + 1);
      }
      if (tpcmoneventsebdc[i]->GetBinContent(tpcmoneventsebdc[i]->GetMaximumBin()) > horiz_max)
      {
        horiz_max = tpcmoneventsebdc[i]->GetBinContent(tpcmoneventsebdc[i]->GetMaximumBin());
      }
    }
  }
  ...
  for (int i = 0; i < 24; i++)
  {
    for (int k = 0; k < 3; k++)
    {
      if (tpcmon_PacketType[i][k])
      {
        ...
        if (tpcmon_PacketType[i][k]->GetEntries() > norm_highest)
        {
          norm_highest = (tpcmon_PacketType[i][k]->GetEntries());
          index_highest = k;
        }
      }
    }
    ...
    for (int j = 0; j < 3; j++)
    {
      if (tpcmon_PacketType[i][index_arr[j]])
      {
        ...
        if (normfactor > 0)
        {
          tpcmon_PacketType[i][index_arr[j]]->Scale(1 / (normfactor), "width");
        }
        gPad->SetLogy(kTRUE);
        if (j == 0)
        {
          tpcmon_PacketType[i][index_arr[j]]->DrawCopy("HIST");
        }
        else
        {
          tpcmon_PacketType[i][index_arr[j]]->DrawCopy("HISTsame");
        }
      }
    }
  ...
  for (int i = 0; i < 24; i++)
  {
    ...
    for (int j = 2; j > -1; j--)
    {
      if (tpcmon_DriftWindow_shifter[i][j])
      {
        for (int k = 1; k < tpcmon_DriftWindow_shifter[i][j]->GetNbinsX(); k++)
        {
          ...
          if (tpcmon_DriftWindow_shifter[i][j]->GetBinContent(k) > max && tpcmon_DriftWindow_shifter[i][j]->GetBinContent(k) > 0)
          {
            max = tpcmon_DriftWindow_shifter[i][j]->GetBinContent(k);
          }
          if (tpcmon_DriftWindow_shifter[i][j]->GetBinContent(k) < min && tpcmon_DriftWindow_shifter[i][j]->GetBinContent(k) > 0)
          {
            min = tpcmon_DriftWindow_shifter[i][j]->GetBinContent(k);
          }
        }
      }
    }

    if (tpcmon_DriftWindow_shifter[i][0])
    {
      R1_max = tpcmon_DriftWindow_shifter[i][0]->GetBinCenter(tpcmon_DriftWindow_shifter[i][0]->GetMaximumBin());
    }
    if (tpcmon_DriftWindow_shifter[i][1])
    {
      R2_max = tpcmon_DriftWindow_shifter[i][1]->GetBinCenter(tpcmon_DriftWindow_shifter[i][1]->GetMaximumBin());
    }
    if (tpcmon_DriftWindow_shifter[i][2])
    {
      R3_max = tpcmon_DriftWindow_shifter[i][2]->GetBinCenter(tpcmon_DriftWindow_shifter[i][2]->GetMaximumBin());
    }

    if ((R1_max > std::numeric_limits<int>::min() && (R1_max < 365)) || R1_max > 374)
    {
      R1_bad = 1;
    }  // old peak - 416, new peak 403 50 ns clock, new peak 368 57.14 ns clock
    if ((R2_max > std::numeric_limits<int>::min() && (R2_max < 365)) || R2_max > 374)
    {
      R2_bad = 1;
    }
    if ((R3_max > std::numeric_limits<int>::min() && (R3_max < 365)) || R3_max > 374)
    {
      R3_bad = 1;
    }
    ...
    for (int l = 2; l > -1; l--)
    {
      if (tpcmon_DriftWindow_shifter[i][l])
      {
        ...
        if (l == 2)
        {
          tpcmon_DriftWindow_shifter[i][l]->GetYaxis()->SetRangeUser(0.9 * min, 1.3 * max);
          tpcmon_DriftWindow_shifter[i][l]->DrawCopy("HIST");
        }  // used to be
        else
        {
          tpcmon_DriftWindow_shifter[i][l]->GetYaxis()->SetRangeUser(0.9 * min, 1.3 * max);
          tpcmon_DriftWindow_shifter[i][l]->DrawCopy("HISTsame");
        }  // assumes that R3 will always exist and is most entries
      }
    }
    ...
    */
    if (tpcmoneventsebdc[i])
    {
      ...
      {
        messages[i]->AddText("NOT ENOUGH STATS");
        ((TText *) messages[i]->GetListOfLines()->Last())->SetTextColor(kBlue);
        messages[i]->AddText("CHECK AGAIN");
        ((TText *) messages[i]->GetListOfLines()->Last())->SetTextColor(kBlue);
        MyTC->cd(i + 5);
        messages[i]->Draw("same");
      }
    ...
    else if ((R1_bad == 1 || R2_bad == 1) || R3_bad == 1)
    {
      ...
      messages[i]->AddText(bad_message);
      ((TText *) messages[i]->GetListOfLines()->Last())->SetTextColor(kBlack);
      messages[i]->AddText("REFRESH. IF PERSISTS, CALL EXPERT");
      ((TText *) messages[i]->GetListOfLines()->Last())->SetTextColor(kBlack);
      MyTC->cd(i + 5);
      messages[i]->Draw("same");
    }
    ...
    {
      messages[i]->AddText("MISSING DATA");
      ((TText *) messages[i]->GetListOfLines()->Last())->SetTextColor(kBlack);
      MyTC->cd(i + 5);
      messages[i]->Draw("same");
    }
...
int TpcMonDraw::DrawShifterTransmissionDist(const std::string & /* what */)
{
  ...
  for (int sector = 0; sector < 24; sector++)
  {
    ...
    {
      if ((!tpcmon_Drift[sector][rad] || tpcmon_Drift[sector][rad]->GetEntries() < 20000))
      {
        continue;
      }  // if doesn't exist move on
  ...
  if (PERCENT->GetEntries() > 36)
  {
    PERCENT->Fit("gaus");
    if (PERCENT->GetFunction("gaus") != nullptr)
    {
    t2->DrawLine(PERCENT->GetFunction("gaus")->GetParameter(1), 0, PERCENT->GetFunction("gaus")->GetParameter(1), 1.05 * PERCENT->GetMaximum());
    }
  }
...
int TpcMonDraw::DrawDCSAMPAvsTIME(const std::string & /* what */)
{
  ...
  tt1->SetTextSize(0.05);
  TText *tt2 = new TText();
  tt2->SetTextSize(0.025);  // fee labels
  ...
  for (int i = 0; i < 24; i++)
  {
    if (tpcmon_dc_sampa_vs_time_plots[i])
    {
      ...
      for (int k = 0; k < 26; k++)
      {
        ...
        if (FEEid[k] > 9)
        {
          tt2->DrawText(-250000, k * 8 + 2, title);
        }  // rollover
        else
        {
          tt2->DrawText(-250000, k * 8 + 2, title);
...
void TpcMonDraw::add_TH1(TH1 *hist[48], TH1 *histadd[24])
{
  for (int i = 0; i < 24; ++i)
  {
    ...
    if (!hist[i] && hist[i + 24])
    {
      histadd[i] = (TH1 *) hist[i + 24]->Clone(Form("histadd_%d", i));
    }
    if (hist[i] && hist[i + 24])
    {
      histadd[i] = (TH1 *) hist[i]->Clone(Form("histadd_%d", i));
...
void TpcMonDraw::add_TH2(TH2 *hist[48], TH2 *histadd[24])
{
  for (int i = 0; i < 24; ++i)
  {
    ...
    if (!hist[i] && hist[i + 24])
    {
      histadd[i] = (TH2 *) hist[i + 24]->Clone(Form("histadd_%d", i));
    }
    if (hist[i] && hist[i + 24])
    {
      histadd[i] = (TH2 *) hist[i]->Clone(Form("histadd_%d", i));
...
void TpcMonDraw::add_TH1_modules(TH1 *hist[48][3], TH1 *histadd[24][3])
{
  for (int i = 0; i < 24; ++i)
  {
    for (int j = 0; j < 3; ++j)
    {
      ...
      if (!hist[i][j] && hist[i + 24][j])
      {
        histadd[i][j] = (TH1 *) hist[i + 24][j]->Clone(Form("histadd_%d_%d", i, j));
      }
      if (hist[i][j] && hist[i + 24][j])
      {
        histadd[i][j] = (TH1 *) hist[i][j]->Clone(Form("histadd_%d_%d", i, j));
...
void TpcMonDraw::add_TH2_modules(TH2 *hist[48][3], TH2 *histadd[24][3])
{
  for (int i = 0; i < 24; ++i)
  {
    for (int j = 0; j < 3; ++j)
    {
      ...
      if (!hist[i][j] && hist[i + 24][j])
      {
        histadd[i][j] = (TH2 *) hist[i + 24][j]->Clone(Form("histadd_%d_%d", i, j));
      }
      if (hist[i][j] && hist[i + 24][j])
      {
        histadd[i][j] = (TH2 *) hist[i][j]->Clone(Form("histadd_%d_%d", i, j));
