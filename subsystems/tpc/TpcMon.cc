...
int TpcMon::Init()
{
  ...
  for (int i = 0; i != 7; ++i)
  {
    Packet_Type_Fraction_HB->GetXaxis()->SetBinLabel(i + 1, label[i]);
    Packet_Type_Fraction_NORM->GetXaxis()->SetBinLabel(i + 1, label[i]);
    Packet_Type_Fraction_ELSE->GetXaxis()->SetBinLabel(i + 1, label[i]);
  }
  ...
  for (int i = 0; i != 7; ++i)
  {
    Packet_Type_vs_sample_ADC->GetYaxis()->SetBinLabel(i + 1, label[i]);
  }
    ...
    else
    {
      ...
      if (p->iValue(0, "NR_DC") != 0)  // && evtcnt > 9000)
      {
        ...
        for (int i = 0; i < p->iValue(0, "NR_DC"); i++)
        {
          ...
          if (starting_BCO_DC < 0)
          {
            starting_BCO_DC = p->iValue(i, "BCO_DC");
          }
      ...
      for (int ci = 0; ci < 6656; ci++)
      {
        Channels_Always->Fill(ci);
      }
      ...
      for (int wf = 0; wf < nr_of_waveforms; wf++)
      {
        ...
        for (int t = 0; t < n_tagger; t++)
        {
          ...
          if (is_lvl1_tagger)
          {
            LVL_1_TAGGER_per_EBDC->Fill(ebdc_from_serverid(MonitorServerId()));
          }
        }
        ...
        if (p->iValue(wf, "TYPE") != 0)
        {
          ...
          if (checksumError == 1)
          {
            Check_Sum_Error->Fill(FEE_transform[fee] * 8 + sampaAddress);
          }
          if (parityError == 1)
          {
            Parity_Error->Fill(FEE_transform[fee] * 8 + sampaAddress);
          }
        }

        if ((checksumError == 0 && parityError == 0) && p->iValue(wf, "TYPE") != 0)
        {
          Channels_in_Packet->Fill(channel + (256 * FEE_transform[fee]));
        }  // do not fill for heartbeat WFs
        ...
        */
        ...
        if (nr_Samples > 0)
        {
          ...
          for (int si = 0; si < nr_Samples; si++)  // get pedestal and noise before hand
          {
            ...
            {
              start_flag = 1;
              if (first_non_ZS_sample == 1)
              {
                First_ADC_vs_First_Time_Bin->Fill(si, (p->iValue(wf, si)));
                first_non_ZS_sample = 0;
              }  // this is the first sample, its all we want
            }
            if ((p->iValue(wf, si)) > 64500 && prev_sample < 1025)
            {
              tr_samp = 0;
              start_flag = 0;
              prev_sample = (p->iValue(wf, si));
            }  // end condition to record
            if (start_flag == 1)
            {
              ZS_Trigger_ADC_vs_Sample->Fill(tr_samp, p->iValue(wf, si));
              tr_samp++;
              prev_sample = p->iValue(wf, si);
            }  // record the ZS trigger histo if you should

            if ((p->iValue(wf, si)) > 64500 && si > 1023)
            {
              break;
            }  // for new firmware/ZS mode - we don't entries w/ ADC > 65 K after 1023 (50 us window), that's nonsense - per Jin's suggestion once you see this, BREAK out of loop
            if ((p->iValue(wf, si)) > 64500)
            {
              continue;
            }  // only use reasonable values to calculate median
        ...
        if (rawnoise == 0. && median_and_stdev_vec.size() > 1)
        {
          ...
          if (stuck_channel_count[channel][FEE_transform[fee]] == 1)
          {
            Stuck_Channels->Fill(FEE_transform[fee]);
          }  // only count # of unique channels in FEE that get stuck at least once
          is_channel_stuck = 1;
        }
        ...
        for (int s = 0; s < nr_Samples; s++)
        {
          ...
          {
            break;
          }  // for new firmware/ZS mode - we don't entries w/ ADC > 65 K after 1023 (50 us), that's nonsense - per Jin's suggestion once you see this, BREAK out of loop
          else if (adc > 64500)
          {
            continue;
          }  // we do not care about 65K ADC entries - ignore them
          ...
          if (adc > wf_max)
          {
            wf_max = adc;
            t_max = s;
            pedest_sub_wf_max = adc - pedestal;
          }

          if ((s > 362 && s < 374) && (adc > wf_max_laser_peak))
          {
            wf_max_laser_peak = adc;
            pedest_sub_wf_max_laser_peak = adc - pedestal;
          }  // old peak near 416, new peak near 403-404 wth 50 ns clock, 368 with 57.14 ns clock
          ...
          else
          {
            ...
            if ((adc == max_of_previous_10 && checksumError == 0) && (parityError == 0 && is_channel_stuck == 0))  // if the new value is greater than the previous 9
            {
              ...
              if (Module_ID(fee) == 0)
              {
                MAXADC_1D_R1->Fill(adc - pedestal);
              }  // Raw 1D for R1
              else if (Module_ID(fee) == 1)
              {
                MAXADC_1D_R2->Fill(adc - pedestal);
              }  // Raw 1D for R2
              else if (Module_ID(fee) == 2)
              {
                MAXADC_1D_R3->Fill(adc - pedestal);
              }  // Raw 1D for R3
            }
          }
          ...
          if ((checksumError == 0 && parityError == 0) && is_channel_stuck == 0)
          {
            ...
            if (Module_ID(fee) == 0)
            {
              RAWADC_1D_R1->Fill(adc);
              PEDEST_SUB_1D_R1->Fill(adc - pedestal);
              PEDEST_SUB_ADC_vs_SAMPLE_R1->Fill(s, adc - pedestal);
            }  // Raw/pedest_sub 1D for R1
            if (Module_ID(fee) == 1)
            {
              RAWADC_1D_R2->Fill(adc);
              PEDEST_SUB_1D_R2->Fill(adc - pedestal);
              PEDEST_SUB_ADC_vs_SAMPLE_R2->Fill(s, adc - pedestal);
            }  // Raw/pedest_sub 1D for R2
            if (Module_ID(fee) == 2)
            {
              RAWADC_1D_R3->Fill(adc);
              PEDEST_SUB_1D_R3->Fill(adc - pedestal);
              PEDEST_SUB_ADC_vs_SAMPLE_R3->Fill(s, adc - pedestal);
            }  // Raw/pedest_sub 1D for R3

            if (Module_ID(fee) == 0 && ((adc - pedestal) > std::max(5.0 * noise, 20.)) && layer != 0)
            {
              COUNTS_vs_SAMPLE_1D_R1->Fill(s);
            }  // Drift window in R1
            if (Module_ID(fee) == 1 && ((adc - pedestal) > std::max(5.0 * noise, 20.)) && layer != 0)
            {
              COUNTS_vs_SAMPLE_1D_R2->Fill(s);
            }  // Drift window in R2
            if (Module_ID(fee) == 2 && ((adc - pedestal) > std::max(5.0 * noise, 20.)) && layer != 0)
            {
              COUNTS_vs_SAMPLE_1D_R3->Fill(s);
            }  // Drift window in R3
            ...
            if ((adc - pedestal) > 25)
            {
              num_samples_over_threshold++;
            }
          }

          // increment
          if (serverid >= 0 && serverid < 12)
          {
            North_Side_Arr[Index_from_Module(serverid, fee)] += adc;
          }
          else
          {
            South_Side_Arr[Index_from_Module(serverid, fee) - 36] += adc;
          }
        ...
        if (num_samples_over_threshold > 15)
        {
          NStreaks_vs_EventNo->Fill(evtcnt);
        }
        ...
        if ((serverid < 12 && (pedest_sub_wf_max) > std::max(5.0 * noise, 20.)) && layer != 0)
        {
          if (Module_ID(fee) == 0)
          {
            NorthSideADC_clusterXY_R1->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max);
            NorthSideADC_clusterXY_R1_unw->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R1
          else if (Module_ID(fee) == 1)
          {
            NorthSideADC_clusterXY_R2->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max);
            NorthSideADC_clusterXY_R2_unw->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R2
          else if (Module_ID(fee) == 2)
          {
            NorthSideADC_clusterXY_R3->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max);
            NorthSideADC_clusterXY_R3_unw->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R3
          ...
          if (t_max >= 50 && t_max <= 330)
          {
            z = 1030 - (t_max - 50) * (57.14 * 0.0735);
            NorthSideADC_clusterZY->Fill(z, R * sin(phi), pedest_sub_wf_max);
            NorthSideADC_clusterZY_unw->Fill(z, R * sin(phi));
          }  // 57 clock
        }
        else if ((serverid >= 12 && (pedest_sub_wf_max) > std::max(5.0 * noise, 20.)) && layer != 0)
        {
          if (Module_ID(fee) == 0)
          {
            SouthSideADC_clusterXY_R1->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max);
            SouthSideADC_clusterXY_R1_unw->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R1
          else if (Module_ID(fee) == 1)
          {
            SouthSideADC_clusterXY_R2->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max);
            SouthSideADC_clusterXY_R2_unw->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R2
          else if (Module_ID(fee) == 2)
          {
            SouthSideADC_clusterXY_R3->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max);
            SouthSideADC_clusterXY_R3_unw->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R3
          ...
          {
            z = -1030 + (t_max - 50) * (57.14 * 0.0735);
            SouthSideADC_clusterZY->Fill(z, R * sin(phi), pedest_sub_wf_max);
            SouthSideADC_clusterZY_unw->Fill(z, R * sin(phi));
          }  // 57 clock
        }
        //________________________________________________________________________________
        // XY laser peak
        if ((serverid < 12 && (pedest_sub_wf_max_laser_peak) > std::max(5.0 * noise, 20.)) && ((t_max > 362 && t_max < 374) && (layer != 0)))  // only fill if the laser was the max old peak 416, new 403 w/ 50 clock, 368 w/ 57.14 clock
        {
          if (Module_ID(fee) == 0)
          {
            NorthSideADC_clusterXY_R1_LASER->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max_laser_peak);
          }  // Raw 1D for R1
          else if (Module_ID(fee) == 1)
          {
            NorthSideADC_clusterXY_R2_LASER->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max_laser_peak);
          }  // Raw 1D for R2
          else if (Module_ID(fee) == 2)
          {
            NorthSideADC_clusterXY_R3_LASER->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max_laser_peak);
          }  // Raw 1D for R3
        }
        else if ((serverid >= 12 && (pedest_sub_wf_max_laser_peak) > std::max(5.0 * noise, 20.)) && ((t_max > 362 && t_max < 374) && (layer != 0)))  // only fill if the laser was the max old peak 416, new 403 w/ 50 clock, 368 w/ 57.14 clock
        {
          if (Module_ID(fee) == 0)
          {
            SouthSideADC_clusterXY_R1_LASER->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max_laser_peak);
          }  // Raw 1D for R1
          else if (Module_ID(fee) == 1)
          {
            SouthSideADC_clusterXY_R2_LASER->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max_laser_peak);
          }  // Raw 1D for R2
          else if (Module_ID(fee) == 2)
          {
            SouthSideADC_clusterXY_R3_LASER->Fill(R * cos(phi), R * sin(phi), pedest_sub_wf_max_laser_peak);
          }  // Raw 1D for R3
        }
        //________________________________________________________________________________
        // 5 event displays
        if ((serverid < 12 && (pedest_sub_wf_max) > std::max(5.0 * noise, 20.)) && layer != 0)
        {
          if (Module_ID(fee) == 0)
          {
            NorthSideADC_clusterXY_R1_u5->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R1
          else if (Module_ID(fee) == 1)
          {
            NorthSideADC_clusterXY_R2_u5->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R2
          else if (Module_ID(fee) == 2)
          {
            NorthSideADC_clusterXY_R3_u5->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R3
        }
        if ((serverid >= 12 && (pedest_sub_wf_max) > std::max(5.0 * noise, 20.)) && layer != 0)
        {
          if (Module_ID(fee) == 0)
          {
            SouthSideADC_clusterXY_R1_u5->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R1
          else if (Module_ID(fee) == 1)
          {
            SouthSideADC_clusterXY_R2_u5->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R2
          else if (Module_ID(fee) == 2)
          {
            SouthSideADC_clusterXY_R3_u5->Fill(R * cos(phi), R * sin(phi));
          }  // Raw 1D for R3
        }
        //________________________________________________________________________________
...
int TpcMon::Module_ID(int fee_id)  // for simply determining which module you are in (doesn't care about sector)
{
  ...
  if (fee_id == 2 || fee_id == 3 || fee_id == 4 || fee_id == 13 || fee_id == 16 || fee_id == 17)
  {
    mod_id = 0;
  }  // R1

  else if (fee_id == 0 || fee_id == 1 || fee_id == 11 || fee_id == 12 || fee_id == 14 || fee_id == 15 || fee_id == 18 || fee_id == 19)
  {
    mod_id = 1;
  }  // R2

  else if (fee_id == 5 || fee_id == 6 || fee_id == 7 || fee_id == 8 || fee_id == 9 || fee_id == 10 || fee_id == 20 || fee_id == 21 || fee_id == 22 || fee_id == 23 || fee_id == 24 || fee_id == 25)
  {
    mod_id = 2;
  }  // R3
...
int TpcMon::ebdc_from_serverid(int server_id)  // for getting the ebdc from the serverid with the double rcdaq
{
  ...
  else if (server_id >= 24)
  {
    ebdc_id = server_id - 24;
  }
...
bool TpcMon::side(int server_id)
{
  ...
  {
    side_id = 1;
  }  // side = 1 when serverid 12 or more
