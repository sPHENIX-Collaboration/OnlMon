...
BcoDiff::BcoDiff(std::string const& name) : SingleInttDrawer(name)
{
}
...
int BcoDiff::DrawCanvas()
{
  MakeCanvas();
  m_canvas->SetTitle("Felix Fphx Bco (Triggered)");
  return SingleInttDrawer::DrawCanvas();
}

int BcoDiff::DrawHist(int fsv)
{
  for (int fch = 0; fch < 14; ++fch)
  {
    if (!m_hist[fsv][fch])
    {
      ...
      m_hist[fsv][fch]->SetTitle((boost::format("intt%01d;Felix BCO - FPHX BCO;Counts (Hits)") % fsv).str().c_str());
  ...
  for (int fch = 0; fch < 14; ++fch)
  {
    ...
    {
      int bincont = bco_hist->GetBinContent(bco_hist->GetBin(1, fch * 128  + bco + 1));
