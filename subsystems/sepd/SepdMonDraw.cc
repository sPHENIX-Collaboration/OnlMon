...
int SepdMonDraw::MakeCanvas(const std::string &name)
{
  ...
  if (gROOT->FindObject("SepdMon6"))
  {
    ...
    TC[canvasindex] -> Draw();
    gSystem->ProcessEvents();
    Pad[16] = new TPad("sepdpad16", "packet event check", 0.0, 0.0, 0.78, 0.95, 0);
    Pad[16]->SetLeftMargin(0.07);
    Pad[16]->SetRightMargin(0.05);
    Pad[16]->Draw();
    Pad[17] = new TPad("sepdpad17", "packet event check legend", 0.75, 0.3, 0.95, 0.95, 0);
    //Pad[2]->SetLeftMargin(0.05);
    Pad[17]->SetRightMargin(0);
    Pad[17] -> Draw();
    //this one is used to plot the run number on the canvas
    transparent[canvasindex] = new TPad("transparent1", "this does not show", 0, 0, 1., 1);
    transparent[canvasindex]->SetFillStyle(4000);
    ...
    // packet warnings
    warning[2] = new TPad("warning1", "packet warnings", 0.75, 0.1, 1, 0.3);
